...
class LabelViewModel : ViewModel() {
     ...
     *
     * * Print a 30*20mm small label with content only
     * SUNMI printers can construct label content according to the relationship of 1mm = 8 pixels
     */
    fun printLabel1() {
        Log.w("LabelViewModel","printLabel1")
        selectPrinter?.canvasApi()?.run {
            ...
            renderText("brand：SUNMI", TextStyle.getStyle().setTextSize(32).enableBold(true).setPosX(10).setPosY(10))
            renderText("SUNMI is an IoT technology company with \"altruism\" as its core value", TextStyle.getStyle().setTextSize(20).setPosX(60)
            ...
            printCanvas(count.value?.toInt()?:1, object : PrintResult() {
                override fun onResult(resultCode: Int, message: String?) {
                    if(resultCode == 0) {
                        //打印完成 -> print complete
                        Log.w("LabelViewModel","print complete")
                    } else {
                        //打印失败 ->print failed
    ...
    /**
     * 打印一张商品信息标签 -> Print a product information label
     */
    fun printLabel2() {
        Log.w("LabelViewModel","printLabel2")
        selectPrinter?.canvasApi()?.run {
            ...
            renderText("Coca Cola(2L)", TextStyle.getStyle().setTextSize(30).enableBold(true)
            ...
            renderText("bottle", TextStyle.getStyle().setTextSize(24)
    ...
    /**
     * 打印一张品牌说明标签 -> Print a branding label
     */
    fun printLabel3(view: View) {
        Log.w("LabelViewModel","printLabel3")
        selectPrinter?.canvasApi()?.run {
