...
/**
 * 内置虚拟蓝牙默认配对 -> Built-in virtual Bluetooth default pairing
 */
object BluetoothUtil {
        ...
        if (bluetoothSocket == null) {
            if (bTAdapter == null) {
               // Toast.makeText(context, "蓝牙设备不可用", Toast.LENGTH_SHORT).show()
                Toast.makeText(context, "bluetooth device not available", Toast.LENGTH_SHORT).show()
                return false
            }
            if (!bTAdapter!!.isEnabled) {
                //Toast.makeText(context, "未检测到蓝牙设备，请打开蓝牙", Toast.LENGTH_SHORT).show()
                Toast.makeText(context, "No bluetooth device detected, please turn on bluetooth", Toast.LENGTH_SHORT).show()
                return false
            }
    ...
    /**
     * 蓝牙方式打印均使用epson指令 -> Bluetooth printing uses the epson command
     *
     * @param bytes
     * @throws IOException
     */
